...
import { VariantManager, type Variant } from "@/components/product/variant-manager";
      ...
      const response = await fetch(`/api/products`, {
        ...
        body: JSON.stringify({
          ...
          compareAtPrice: formData.compareAtPrice
            ...
            : null,
          costPrice: formData.costPerItem
            ? parseFloat(formData.costPerItem)
            : null, // Changed from costPerItem
          inventoryQty: parseInt(formData.stock) || 0, // Changed from stock
          status: formData.status,
          images: JSON.stringify(images), // Include images
          variants: variants.map((v) => ({
            name: v.name,
            sku: v.sku || `${formData.sku || "SKU"}-${Date.now()}`,
            price: v.price || null,
